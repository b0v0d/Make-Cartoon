# Make-Cartoon
Cartoonize real world! dive in cartoon world!
...
### 4. 코드의 한계 분석
변환이 이루어진 케이스도 사실 완벽한 변환이 된 것은 아니다. 손의 오른 쪽 부분을 보면, 빛을 받은 손이 배경의 색과 거의 흡사하여 경계로 인식되지 않았다. 이는 그림의 전체 문맥을 보고 종합적으로 판단하는 사람과 달리, 단순히 인접한 영역의 색차이를 보고 경계를 판단하는 알고리즘의 한계라고 생각한다. 변환이 잘 이루어지지 않은 경우 맨 앞에 있는 나무는 선명하여 결과가 뚜렷한 반면, 상대적으로 뒤에 있는 나무들은 흐리기 때문에 openCV의 알고리즘이 인식을 하지 못한 것이라 판단된다.
